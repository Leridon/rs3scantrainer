...
import type { MapPoint, CoordMeta } from "./textclue";
...
function isArcClue(buf: ImageData) {
	var n = 0;
...
function getdirection(buf: ImageData) {
	//var size = 170;
	var maxr = 60;
	var minr2 = 20;
	var maxr2 = 50;
	...
	for (let opt of newAngleTrans) {
		...
		if (d <= 0 && d > dunder) { under = opt; dunder = d; }
		if (d >= 0 && d < dabove) { above = opt; dabove = d; }
	}
