...
class ScanTreeSolvingControl extends Behaviour {
    ...
    private renderLayer(): void {
        ...
        AugmentedScanTree.collect_parents(node, false).forEach(n => {
            ...
            PathGraphics.renderPath(n.raw.path, this.parent.app.data.teleports).setOpacity(0.2).addTo(this.layer)
        })

        // Children paths to dig spots are rendered with 0.5
        node.children.forEach(c => {
            PathGraphics.renderPath(c.value.raw.path, this.parent.app.data.teleports).setOpacity(0.5).addTo(this.layer)
